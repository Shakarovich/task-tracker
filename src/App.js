...
function App() {
  ...
  const [completed, setCompleted] = useState([])
    ...
    const [{isOver}, drop] = useDrop(() => ({
        accept: "todo",
        drop: (item) => addToCompeted(item.id, item.projectName, item.taskDescription, item.timestamp, item.duration),
        collect: (monitor) => ({
            isOver: !!monitor.isOver()
        })
    }))

const addToCompeted  = (id, projectName, taskDescription, timestamp, duration) => {
        ...
        <div className="flex flex-row">
            <div className="w-full">
                ...
                <div className="ml-6 flex flex-col-reverse">
                    {taskList.map((task,i) => (
                        <ToDo key={i} index={i} task={task} taskList={taskList} setTaskList={setTaskList}/>
                    ))}
                </div>
            </div>
            <div className="w-full flex flex-col" ref={drop}>
                <h2 className="text-xl font-semibold w-3/4  max-w-lg py-2 px-2 my-4 bg-gray-300">Completed:</h2>
